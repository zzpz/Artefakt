...
interface CheckBoxProps extends RoadmapItem {
    index: number;
}
...
export const Roadmap = (props?: RoadmapProps) => {
    ...
    const front: RoadmapItem[] =
        [
            ...
            { checked: true, description: "FileUpload Client", state: "fragile" },
    ...
    const frontItems = items.map((item, index) => {
        return (<MyCheckBox index={index} id={item.id} checked={item.checked} description={item.description} />)
    })
